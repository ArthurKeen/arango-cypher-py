...
_SCENARIOS = _collect_scenarios(["clauses/unwind"], "unwind")
...
def test_tck_unwind_scenario(feature_name, scenario):
    if os.environ.get("RUN_TCK") != "1":
        pytest.skip("Set RUN_TCK=1")
    outcome = run_scenario(scenario, db_name="tck_unwind_db", mapping_fixture="lpg")
