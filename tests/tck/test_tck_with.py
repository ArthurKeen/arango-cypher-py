...
_SCENARIOS = _collect_scenarios(
    ["clauses/with", "clauses/with-orderBy", "clauses/with-skip-limit", "clauses/with-where"],
    "with",
)
...
def test_tck_with_scenario(feature_name, scenario):
    if os.environ.get("RUN_TCK") != "1":
        pytest.skip("Set RUN_TCK=1")
    outcome = run_scenario(scenario, db_name="tck_with_db", mapping_fixture="lpg")
