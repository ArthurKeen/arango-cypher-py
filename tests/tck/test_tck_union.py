...
_SCENARIOS = _collect_scenarios(["clauses/union"], "union")
...
def test_tck_union_scenario(feature_name, scenario):
    if os.environ.get("RUN_TCK") != "1":
        pytest.skip("Set RUN_TCK=1")
    outcome = run_scenario(scenario, db_name="tck_union_db", mapping_fixture="lpg")
