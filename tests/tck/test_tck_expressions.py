...
def _collect_scenarios(subdirs: list[str], pattern: str):
    ...
    for subdir in subdirs:
        ...
        for feat_file in sorted(feat_dir.rglob("*.feature")):
            if pattern and pattern.lower() not in feat_file.name.lower():
                continue
...
def test_tck_expression_scenario(feature_name, scenario):
    if os.environ.get("RUN_TCK") != "1":
        pytest.skip("Set RUN_TCK=1")
    outcome = run_scenario(scenario, db_name="tck_expr_db", mapping_fixture="lpg")
