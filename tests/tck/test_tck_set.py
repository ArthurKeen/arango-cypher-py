...
_SCENARIOS = _collect_scenarios(["clauses/set", "clauses/remove"], "")
...
def test_tck_set_scenario(feature_name, scenario):
    if os.environ.get("RUN_TCK") != "1":
        pytest.skip("Set RUN_TCK=1")
    outcome = run_scenario(scenario, db_name="tck_set_db", mapping_fixture="lpg")
