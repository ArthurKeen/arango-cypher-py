...
> **Historical (2026-04).** The analyzer repo referred to below as `arango-schema-mapper` is now `arango-schema-analyzer` (package `arangodb-schema-analyzer`); the supported version band lives in `pyproject.toml` (see `docs/PRD.md` §7.1). Mentions below are left as written. *(Banner added 2026-09-15.)*
