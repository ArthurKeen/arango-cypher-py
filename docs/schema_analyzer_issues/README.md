...
| [06](./06-cheap-shape-fingerprint-probe.md) | Add cheap `fingerprint_physical_shape(db)` / `fingerprint_physical_counts(db)` (no snapshot required) | `_shape_fingerprint`, `_full_fingerprint`, `_index_digest`, `_iter_user_collections` (~60 LOC) | shipped in v0.3.0 | PR-2 |
