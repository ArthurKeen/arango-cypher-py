...
|---------|--------|--------|
| **CORS** | Allow configurable origins via `CORS_ALLOWED_ORIGINS` env var (comma-separated, or `*`). Default: `*` for dev convenience -- pin to an explicit allowlist in production. When `*` is combined with `allow_credentials=True`, browsers reflect the requested origin rather than echoing `*`. Documented in `.env.example`. | Implemented |
